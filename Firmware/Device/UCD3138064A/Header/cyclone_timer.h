...
extern "C" {
  ...
  struct T16PWMXCMPCTRL_BITS {     // bits   description
    ...
    Uint32 SHADOW:1;               // 12     PWM update control
    Uint32 PWM_IN:1;               // 11     [pin=64]TPWMx pin state
    Uint32 PWM_OUT:1;              // 10     [pin=64]TPWMx pin output data
    Uint32 PWM_OUT_ENA:1;          // 9      [pin=64]TPWMx pin output enable
    Uint32 PWM_OUT_DRV:1;          // 8      [pin=64]TPMWx pin output control
    Uint32 PWM_OUT_ACTION1:2;      // 7:6    [pin=64]TPWMx pin output action on T16CMP1DR match
    Uint32 PWM_OUT_ACTION0:2;      // 5:4    [pin=64]TPWMx pin output action on T16CMP0DR match
    Uint32 CMP1_INT_ENA:1;         // 3      Compare 1 interrupt enable
    Uint32 CMP1_INT_FLAG:1;        // 2      Compare 1 interrupt flag
    Uint32 CMP0_INT_ENA:1;         // 1      Compare 0 interrupt enable
    Uint32 CMP0_INT_FLAG:1;        // 0      Compare 0 interrupt flag
